...
export interface FormatTimeSinceOptions {
  now?: Date
}
 ...
 *
 * Accepts a Date instance, ISO-like string, or epoch milliseconds number.
 * The output is English-only and returns phrases like "5 seconds ago" or
 * "in 3 hours". Callers that need localisation should wrap this helper and
 * map its behaviour to translation keys.
 */
export function formatTimeSince(input: TimeSinceInput, options?: FormatTimeSinceOptions): string {
  if (input == null) {
    throw new Error('formatTimeSince: invalid date input')
  }
  ...
  if (seconds < 60) {
    ...
    value = Math.max(1, Math.floor(days / 7))
    unit = 'week'
  } else if (days < 365) {
    value = Math.max(1, Math.floor(days / 30))
    unit = 'month'
  } else {
    value = Math.max(1, Math.floor(days / 365))
    unit = 'year'
  }
