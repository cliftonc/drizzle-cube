...
function ValueLabel(props: any) {
  const { x, y, width, value, runningBase, isNegative, displayValue } = props
  ...
  return (
    <text
      ...
      textAnchor="middle"
      fontSize={11}
      className="dc:fill-current text-dc-text-primary"
    >
...
const WaterfallChart = React.memo(function WaterfallChart({
  ...
  const showTotal: boolean = displayConfig?.showTotal ?? true
  const showConnectorLine: boolean = displayConfig?.showConnectorLine ?? true
  ...
  const { xAxisField, yAxisField, configError } = useMemo(() => {
    const xAxisField = Array.isArray(chartConfig?.xAxis)
      ? chartConfig.xAxis[0]
      : chartConfig?.xAxis ?? chartConfig?.x
    const yAxisField = Array.isArray(chartConfig?.yAxis)
      ? chartConfig.yAxis[0]
      : chartConfig?.yAxis?.[0] ?? chartConfig?.y
    const configError =
        ...
        <ComposedChart data={chartData} margin={{ ...CHART_MARGINS, left: 40 }} accessibilityLayer={false}>
          ...
          <ChartTooltip
            formatter={(value: any, name: any, props: any) => {
              if (name === '_connector') return null
          ...
          <Legend
            ...
            payload={[
              { value: 'Increase', type: 'rect', color: POSITIVE_COLOR },
              { value: 'Decrease', type: 'rect', color: NEGATIVE_COLOR },
