...
| mcp-app-html.ts | `mcpAppHtml` | The bundled ~2 MB MCP App HTML as its own public subpath (`drizzle-cube/mcp-app-html`). Only `mcp-transport.ts` (dynamic `import()`, in `getMcpAppHtml`) and `mcp-tools.ts` (static, its `resources` are sync API) may reach it — never a static import from adapter code. |
...
1. **Security context is mandatory** — `extractSecurityContext` must be provided; queries cannot execute without it. Since per-tenant cube sets, this covers *every* endpoint that reads a cube list, not just query execution: `/meta`, MCP `resources/list` + `resources/read`, and the MCP `discover` / `validate` tools all resolve a context first and there is no base-set fallback. `handleDiscover`, `handleSuggest` and `handleValidate` take a **required** `SecurityContext` as their second argument (matching `handleLoad`).
2. **Every REST response is uncacheable by shared caches** — the core stamps `Cache-Control: private, no-store` on all of `/meta`, `/load`, `/sql`, `/dry-run`, `/batch`, `/explain` via `withRestCacheHeaders`. Never send a REST response around it.
3. **API consistency** — all four adapters expose identical endpoints and response shapes.
4. **MCP origin validation** — `validateOriginHeader` in mcp-transport.ts enforces allowed-origin checks before processing MCP requests (POST via the core handler; GET/DELETE inline in each adapter). Default policy (no `mcp.allowedOrigins`): admit loopback origins + no-Origin server-to-server clients (Claude connector, curl); reject other browser origins with 403 to block DNS rebinding. Browser front-ends must be listed in `mcp.allowedOrigins`; `['*']` restores permissive mode. Use `originOptionsFromMcp(mcp)` to derive the options consistently. For public deployments, `resourceMetadataUrl` (Bearer auth) is the primary control; origin validation is defense-in-depth.
5. **MCP App HTML is lazy** — the ~2 MB single-file MCP App (`src/mcp-app/generated-html.ts`) must only be reached from the adapters through `getMcpAppHtml()`'s dynamic `import('./mcp-app-html.js')`, loaded on the first `resources/list` / `resources/read` with `mcp.app` on. A static import anywhere on the adapter graph (`hono`/`express`/`fastify`/`nextjs`/`core`/`utils`/`mcp-transport`) puts the blob in every consumer's server bundle even with `mcp.enabled: false` — this is what cost Cloudflare Workers users ~1 MB gzip. `vite.config.adapters.ts` rewrites that one `import()` to the bare specifier `drizzle-cube/mcp-app-html` so bundlers can alias it to a stub; `tests/adapters/mcp-app-html-lazy.test.ts` guards the laziness. `mcp-tools.ts` (`drizzle-cube/mcp`) is the one deliberate static importer.
6. **Locale propagation** — the `X-DC-Locale` header is merged into `SecurityContext` via `withLocaleFromHeaders()` (re-exported from `core`; wraps `resolveRequestLocale` + `withLocaleInSecurityContext` from `locale.ts`). Core handlers do this internally; adapters supply a base (pre-locale) context thunk. For transport-bound handlers an adapter keeps inline (e.g. Express agent/chat), call `withLocaleFromHeaders` directly rather than re-declaring the wrapper. See `src/i18n/CLAUDE.md`.
