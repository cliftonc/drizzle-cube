...
- [ ] **3. Create chart config** — Create `src/client/components/charts/{Name}.config.ts` (note: `.config.ts`, not `.config.tsx`). Export a named `{name}ChartConfig` of type `ChartTypeConfig` (from `src/client/charts/chartConfigs.ts`). Define `dropZones` (array of `AxisDropZoneConfig`), `displayOptions` or `displayOptionsConfig`, `icon` (via `getChartTypeIcon`), `description`, `useCase`, and optional `validate` function. Set `skipQuery: true` only for content-only charts (markdown, KPIs).
