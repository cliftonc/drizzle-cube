...
aiApp.post('/generate', async (c) => {
  ...
  try {
    ...
    if (typeof requestBody === 'string') {
      ...
      userPrompt = requestBody.text
    } else if (requestBody.contents) {
      // Gemini format - extract text
      const rawText = requestBody.contents
      ...
      if (!rawText) {
        return c.json({
          error: 'Invalid request body. No text content found in contents'
        }, 400)
      }
      ...
      return c.json({
        error: 'Invalid request body. Provide either "text" or "contents" with user prompt'
      }, 400)
    }
