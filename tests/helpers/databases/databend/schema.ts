/**
 * Test database schema for Databend using Drizzle ORM
 * Databend uses pgcore (extends PgDialect) so this schema mirrors the DuckDB/PostgreSQL test schema
...
// Employee table - Databend version (uses PostgreSQL-compatible types via pgcore)
export const employees = pgTable('employees', {
  ...
  salary: real('salary'),
  tags: varchar('tags', { length: 100 }), // Databend doesn't support arrays - store as comma-separated
  createdAt: timestamp('created_at', { mode: 'date' }).defaultNow()
})

// Department table - Databend version
...
export const databendTestSchema = {
  employees,
  departments,
  productivity,
  timeEntries,
...
export type DatabendTestSchema = typeof databendTestSchema
